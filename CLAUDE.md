...
- **The labels are right-aligned with `justify-self: end` on `.settings-item > :first-child`**, which is the second opt-out from `justify-items: start` after the headings' `stretch`. A `text-align: right` on its own does nothing here — the box is content-sized and already sits at the left edge of the column — so the `text-align` in that rule is only there for a label that wraps to a second line. The selector is `:first-child` and **not `> label`**, because `#manualRefresh`'s label is a `<span>` (see below), and it is safe only because of the exactly-two-children rule above.
